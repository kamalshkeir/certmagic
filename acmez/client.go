...
func (c *Client) ObtainCertificate(ctx context.Context, params OrderParameters) ([]acme.Certificate, error) {
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		if errors.As(err, &problem) {
			...
			pp := []any{"problem", problem, "order", order.Location, "attempt", attempt, "max_attempts", maxAttempts}
			if haveAuthz {
				pp = append(pp, "identifier", authz.IdentifierValue())
			}
			lg.Error("validating authorization", pp...)
	...
	defer func() {
		...
		for _, authz := range authzStates {
			...
			if err := authz.currentSolver.CleanUp(ctx, authz.currentChallenge); err != nil {
				lg.Error("cleaning up solver", "err", err, "identifier", authz.IdentifierValue(), "challenge_type", authz.currentChallenge.Type)
			}
		}
		...
		for _, authz := range authzStates {
			...
			if err != nil {
				lg.Error("deactivating authorization", "err", err, "identifier", authz.IdentifierValue(), "authz", authz.Location)
			}
...
func (c *Client) initiateCurrentChallenge(ctx context.Context, authz *authzState) error {
	if authz.Status != acme.StatusPending {
		lg.Debug("skipping challenge initiation because authorization is not pending", "identifier", authz.IdentifierValue(), "authz_status", authz.Status)
		return nil
	}
	...
	if waiter, ok := authz.currentSolver.(Waiter); ok {
		lg.Debug("waiting for solver to be ready", "identifier", authz.IdentifierValue(), "challenge_type", authz.currentChallenge.Type)
		err := waiter.Wait(ctx, authz.currentChallenge)
		lg.Debug("done waiting for solver", "identifier", authz.IdentifierValue(), "challenge_type", authz.currentChallenge.Type)
		if err != nil {
	...
	if payloader, ok := authz.currentSolver.(Payloader); ok {
		lg.Debug("getting payload from solver", "identifier", authz.IdentifierValue(), "challenge_type", authz.currentChallenge.Type)
		p, err := payloader.Payload(ctx, authz.currentChallenge)
		lg.Debug("done getting payload from solver", "identifier", authz.IdentifierValue(), "challenge_type", authz.currentChallenge.Type)
		if err != nil {
	...
	lg.Debug("challenge accepted", "identifier", authz.IdentifierValue(), "challenge_type", authz.currentChallenge.Type)
	return nil
}
...
func (c *Client) nextChallenge(authz *authzState) error {
	...
	for _, prefChalType := range preferredChallenges {
		for i, remainingChal := range authz.remainingChallenges {
			...
			lg.Debug("no solver configured", "identifier", authz.IdentifierValue(), "challenge_type", remainingChal.Type)
			break
		}
	}
...
func (c *Client) pollAuthorization(ctx context.Context, account acme.Account, authz *authzState, failedChallengeTypes failedChallengeMap) error {
	...
	if authz.currentSolver != nil {
		...
		if cleanupErr != nil {
			lg.ErrorC("cleaning up solver", "identifier", authz.IdentifierValue(), "challenge_type", authz.currentChallenge.Type, "error", cleanupErr)
		}
	...
	if err != nil {
		var problem acme.Problem
		if errors.As(err, &problem) {
			lg.Error("challenge failed", "identifier", authz.IdentifierValue(), "challenge_type", authz.currentChallenge.Type, "error", err)
