	...
	for attempts = 1; attempts <= maxAttempts; attempts++ {
		...
		if errors.As(err, &problem) {
			if problem.Type == ProblemTypeBadNonce {
				...
				lg.Debug("server rejected our nonce; retrying", "err", err, "detail", problem.Detail)
				continue
			}
		}
...
func (c *Client) httpReq(ctx context.Context, method, endpoint string, joseJSONPayload []byte, output any) (*http.Response, error) {
	...
	for attempt := 0; attempt < maxAttempts; attempt++ {
		...
		if err != nil {
			if retry {
				lg.Warn("HTTP request failed; retrying", "url", req.URL.String(), "err", err)
				continue
			}
			break
		}
...
func (c *Client) doHTTPRequest(req *http.Request, buf *bytes.Buffer) (resp *http.Response, retry bool, err error) {
	...
	lg.Debug("http request", "method", req.Method, "url", req.URL.String(), "headers", req.Header, "response_headers", resp.Header, "status_code", resp.StatusCode)
